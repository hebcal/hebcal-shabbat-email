...
/** Renders the note for the desired output format (HTML vs. plain text). */
type NoteBuilder = (isHTML: boolean) => string;

/** Shared date/location context computed once and passed to each seasonal branch. */
type NoteContext = {
  cfg: SpecialNoteConfig;
  mm: number;
  dd: number;
  yy: number;
  purimMonth: number;
  shortLocation: string | null;
  makeUrl: (holiday: string, isHTML: boolean) => string;
};

function noteRoshHashana(ctx: NoteContext): NoteBuilder | undefined {
  const {cfg, mm, dd, yy, shortLocation} = ctx;
...
function noteBetweenRhYk(ctx: NoteContext): NoteBuilder | undefined {
  const {mm, dd, yy, makeUrl} = ctx;
  if (!(mm === months.TISHREI && dd <= 9)) {
    return undefined;
  }
  // between RH & YK
  const erevYK = dayjs(new HDate(9, months.TISHREI, yy).greg());
  const strtime = nowrap(erevYK.format(FORMAT_DOW_MONTH_DAY));
  return isHTML => `G’mar Chatima Tova! We wish you a good inscription in the Book of Life.
<br><a style="color:#356635" href="${makeUrl('yom-kippur', isHTML)}">Yom Kippur ${yy}</a>
begins at sundown on ${strtime}.`;
}

function noteSukkotPesach(ctx: NoteContext): NoteBuilder | undefined {
  const {mm, dd} = ctx;
  if (!(
...
function notePurim(ctx: NoteContext): NoteBuilder | undefined {
  const {mm, dd, yy, purimMonth, makeUrl} = ctx;
  if (!(mm === purimMonth && dd >= 2 && dd <= 10)) {
    return undefined;
  }
  ...
  return isHTML => `Chag Purim Sameach!
<a style="color:#356635" href="${makeUrl('purim', isHTML)}">Purim ${yy}</a>
begins at sundown on ${strtime}.`;
}

function notePesach(ctx: NoteContext): NoteBuilder | undefined {
  const {mm, dd, yy, purimMonth, makeUrl} = ctx;
  if (!(
  ...
  return isHTML => `Chag Kasher v’Sameach! We wish you a happy
<a style="color:#356635" href="${makeUrl('pesach', isHTML)}">Pesach ${yy}</a>.
<br>Passover begins at sundown on ${strtime}.`;
}

function noteChanukah(ctx: NoteContext): NoteBuilder | undefined {
  const {mm, dd, yy, makeUrl} = ctx;
  ...
  return isHTML => `Chag Urim Sameach! Light the first
...
export function getSpecialNote(cfg: SpecialNoteConfig, today: Dayjs): [string, string] {
  ...
  const ctx: NoteContext = {
    ...
    dd: hd.getDate(),
    yy,
    purimMonth: HDate.isLeapYear(yy) ? months.ADAR_II : months.ADAR_I,
    shortLocation: cfg.location.getShortName(),
    makeUrl(holiday: string, isHTML: boolean): string {
